            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
				...
				while($rows = mysqli_fetch_array($result))
				{
					...
					$mat_id = $rows['mat_id'];
					$sql4 = "SELECT match_id FROM earn_money_transaction WHERE `match_id` = '$mat_id' LIMIT 0,1";
					$result4 = mysqli_query($conn,$sql4);
					$row4 = mysqli_fetch_assoc($result4);
					$match_id = $row4['match_id'];
					?>
					<tr>
					  <td><?php echo $i;?></td>
					  <td>
						...
						if ($rows['mat_isPrizeDesctributed'] == 0) {
							...
							<form action="../code/status_change" method="post">
							  ...
							  <input type="hidden" name="link" value="match_completed">
						...
						if ($match_id != $mat_id){
							?>
							<a href="http://matchonsports.com/app_code/FantasyController/distributePromoterBonus/<?php echo $mat_id;?>" class="btn btn-block btn-info">Promoter Distribute</a>
							<?php
						}
						else{
							echo "Distributed";
