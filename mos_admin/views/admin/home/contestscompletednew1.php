...
if(isset($_SESSION["val20"])&&isset($_SESSION["val21"]))
{
$match_id=intval($_SESSION["val20"]);
$contest_id11=intval($_SESSION["val21"]);
}
else
{
...
<?php
echo '
<a href="contestscompletednewt1.php">
...
<div class="card btm mx-auto">
  ...
  <div class="col  text-right float-right">
  <span class="tit"><b>RANK(Rs.Won)</b></span>
  </div>
  </div>
  </div>';
...
$query1="SELECT * FROM `totalpointsfixed` WHERE `teamid`='$user12' and `matchid`='$match_id'";
...
$amount333=array(0.0,0.0,0.0);
for($yu=1;$yu<=$countrank;$yu++)
{

$query111="SELECT max(max1) as cont FROM `prize` WHERE  contestid='$contest_id11' ";
$result111=mysqli_query($con,$query111);
while ($rows111 = mysqli_fetch_array($result111)) 
{
$maxprize=$rows111['cont'];
}
$difu=1;
if($rank333[$yu]<=$maxprize)
{

/*
$query1111="SELECT * FROM `prize` WHERE  contestid='$contest_id11' and min1<='$rank333[$yu]' and max1>='$rank333[$yu]'";
$result1111=mysqli_query($con,$query1111);
while ($rows1111 = mysqli_fetch_array($result1111)) 
{
$minprize1=$rows1111['min1'];
$maxprize1=$rows1111['max1'];
$prize12=$rows1111['prize'];
$difu=$maxprize1-$minprize1+1;

//echo $rank333[$yu]." ".$query1111;
}
*/

$ctcount=0;
for($yu1=1;$yu1<=$countrank;$yu1++)
{
if($rank333[$yu]==$rank333[$yu1])
{
$ctcount++;
}
}
...
for($yu2=0;$yu2<$ctcount;$yu2++)
{
$tt=$yu2+$rank333[$yu];
$query11112="SELECT * FROM `prize` WHERE  contestid='$contest_id11' and min1<='$tt' and max1>='$tt'";
$result11112=mysqli_query($con,$query11112);
while ($rows11112 = mysqli_fetch_array($result11112)) 
{
$prize12=$prize12+$rows11112['prize'];
...
$tyu12=floatval($prize12);
$tyu12=floatval($tyu12/$ctcount);
$amount333[$yu]=floatval($tyu12);
...
$ty1=((intval($user12)+$ty)*3)-$ty;


echo '
<a href="previewcompletednewt.php?trans1='.$ty1.'">
<div class="card btm mx-auto">
  <div class="row">
  <div class="col float-left" style="width: 255px;">
...
echo '
  <span class="tit"><font color="orange">'.$rank333[$indi].'(Rs.'.$amount333[$indi].')</font></span>
  </div>
  </div>
  </div></a>';
...
echo '
  <span class="tit"><font color="black">'.$rank333[$indi].'(Rs.'.$amount333[$indi].')</font></span>
  </div>
  </div>
  </div></a>';
...
echo '
<a href="previewcompletednewt.php?trans1='.$ty2.'">
<div class="card btm mx-auto">
  <div class="row">
  <div class="col float-left" style="width: 255px;">
  <span class="tit"><font color="black">'.$user2.'(T'.$tttcount.')</font></span>
...
echo ' <span class="tit"><font color="black">'.$rank333[$indi].'(Rs.'.$amount333[$indi].')</font></span>
  </div>
  </div>
  </div></a>';
