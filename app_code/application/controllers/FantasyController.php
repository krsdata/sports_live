...
class FantasyController extends CI_Controller 
{
    ...
    public function getPlayers($comp_id=115932, $match_id=44990)
    {
    ...
    private function processSquadPlayer($result, $match_id)
    {
        ...
        if(isset($result['status']) && $result['status'] == "ok" && isset($result['response']['squads'][0]['players']) && isset($result['response']['squads'][1]['players']))
        {
            ...
            foreach ($team as $key => $value) 
            {
                ...
                //player match role
                $ply_data_rol[$key][] = $value['pid'];
                $ply_data_rol[$key][] = $match_id;
                $ply_data_rol[$key][] = $value['playing_role'];

                //set count 0 to players (only insertion)
                $ply_data_sel[$key][] = $value['pid'];
                $ply_data_sel[$key][] = $match_id;
            }
            ...
            //insert or update match
            $this->FantasyModel->savePlayerRole($ply_data_rol);

            //insert match player
            $this->FantasyModel->saveInitialCountOfPlayer($ply_data_sel);
        ...
        foreach ($liveMatchIds as $liveMatchId) 
        {
            ...
            if ($scoreData['status'] == "ok") 
            {
                ...
                if ($scoreData['response']['verified'] == "true") 
                {
                    ...
                    if ($mat_status == 2) 
                    {
                        //process live match actual data
                        $this->processPlayerActual($matchId);

                        //update user rank and prize
    ...
    {
        $url = FANTASY_API_ENDPOINT.'matches/'.$matchId.'/newpoint?'.$this->token;
        ...
        if ($pointData && $pointData['status'] == "ok") 
        {
            ...
            foreach ($matchPlayers as $key => $value) 
            {
                ...
                $ply_point_data[$key][] = $value['bonus'];
            }
    ...
    public function distributePromoterBonus($matchId, $userId)
    {
        $this->FantasyModel->distributePromoterBonus($matchId, $userId);
    }
    ...
    //save players actual data
    public function processPlayerActual($n_matchId='')
    {
        if ($n_matchId) 
            $liveMatchIds = array(array('mat_id' => $n_matchId));
        else
        {
            //get all live matches from table
            $liveMatchIds = $this->CommonModel->selectRecords(array('mat_status' => 3), 'API_matches', 'mat_id');
        ...
        {
            //get score of live match
            $url = FANTASY_API_ENDPOINT.'matches/'.$liveMatchId['mat_id'].'/scorecard?'.$this->token;
            $scoreData = $this->callAPI($url);
            
            if ($scoreData['status'] == "ok" && isset($scoreData['response']['innings'])) 
            {
                $this->FantasyModel->resetMatchPlayersActual($scoreData['response']['match_id']);
                
                //store record for innings
                foreach ($scoreData['response']['innings'] as $inning) 
                {
                    //echo "<pre>"; print_r($inning['batsmen']); die;
                    //save batting players
                    if (isset($inning['batsmen']))
                    {
                        foreach ($inning['batsmen'] as $batsmen) 
                            $this->FantasyModel->saveBatsmanActualData($batsmen);
                    }

                    //save bowling players
                    if (isset($inning['bowlers']))
                    {
                        foreach ($inning['bowlers'] as $bowler) 
                            $this->FantasyModel->saveBowlerActualData($bowler);
                    }

                    //save fielders
                    if (isset($inning['fielder']))
                    {
                        foreach ($inning['fielder'] as $fielder) 
                            $this->FantasyModel->saveFielderActualData($fielder);
                    }
                }

                //get live players point
                $this->getAndProcessPlayerPoints($scoreData['response']['match_id']);
            }
        }
    }
    ...
    {
        $this->FantasyModel->deductDuplicateAutoCancel($data);
    }
}
?>