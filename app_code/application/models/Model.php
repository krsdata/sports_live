...
class Model extends CI_Model
{
    ...
    public function getMatchSquad($matchId, $matchStatus)
    {
        $sql = 'SELECT 
    ...
    public function getMatchContest($request)
    {
        ...
        $sql = 'SELECT 
                    contests.contests_id AS contestId, 
                    contests.amount AS prizeMoney, 
                    fee AS entryFee, 
                    no_of_teams AS totalTeamCount, 
                    MAX(max1) AS totalWinnerCount, 
                    multi AS multipleJoinAllowed, 
                    fill1 AS autoCancelAllowed, 
                    contests.type AS contestType, 
                    contests.matchid AS matchId, 
                    "" AS userRank, 
                    jinku AS bonusPercent,
                    (
                        SELECT prize 
                        FROM prize 
                        WHERE 
                            contestid = contests.contests_id AND 
                            min1 = 1
                    ) AS rank1Prize
                FROM contests
    ...
    public function joinContest($request)
    {
        ...
        $sql = 'SELECT
                ...
                IF((SELECT COUNT(*) FROM matches_joined WHERE contests_id = '.$contestId.' AND match_id = '.$matchId.' AND team_id = '.$teamId.' AND user_id = '.$userId.') = 0, 0, 1) AS isAlreadyJoinedWithOneTeam, jinku AS bonusPercent,
                (CASE 
                    WHEN multi = 1
                        THEN
        ...
        if ($result && $result['isAllowedMultiJoin'] && !$result['isAlreadyJoinedWithOneTeam']) 
        {
            ...
            {
                $isValidAmount = $bamount-(($contestFee*$result['bonusPercent'])/100);
                if ($isValidAmount < 0)
                    $checkAmount = $damount+$wamount+$isValidAmount;
                else
                    $checkAmount = $damount+$wamount;

                if ($checkAmount < ($contestFee-(($contestFee*$result['bonusPercent'])/100))) 
            ...
            if ($inserted_id) 
            {
                ...
                if ($result['contestType'] == 1) 
                {
                    $fixedBamount = ($contestFee*$result['bonusPercent'])/100;
                    $remainAmount = 0;
    ...
    public function getPlayerStats($request)
    {
        ...
        $sql = "SELECT 
                    pnt_id AS pointId, 
                    pnt_ply_id AS playerId, 
                    pnt_total AS points, 
                    pnt_rating AS rating, 
                    pnt_starting11 AS `stating 11`, 
                    pnt_run AS runs, 
                    pnt_four AS `4's`, 
                    pnt_six AS `6's`, 
                    pnt_sr AS `s/r`, 
                    ...
                    pnt_duck AS duck, 
                    pnt_wkts AS wkts, 
                    pnt_maidenover AS `maiden over`, 
                    pnt_er AS `e/r`, 
                    pnt_catch as catch, 
                    pnt_runoutstumping AS `run out/strumping`, 
                    pnt_runoutthrower AS `r/o throw over`, 
                    pnt_runoutcatcher AS `r/o catcher`, 
                    pnt_directrunout AS `r/o direct`, 
                    pnt_stumping AS stumping, 
                    pnt_thirty AS `30`, 
                    pnt_bonus AS bonus, 
                    ply_first_name AS firstName, 
                    ply_middle_name AS middleName, 
                    ply_last_name AS lastName, 
                    ply_title AS title, 
                    ply_shortName AS shortName, 
                    ply_logo_url AS logo, 
                    rol_name AS role
                FROM API_player_point
                INNER JOIN API_players ON ply_id = pnt_ply_id
                LEFT JOIN API_player_role
                    ON rol_ply_id = ply_id AND rol_mat_id = ".$matchId."
    ...
    public function getPlayerSelectByPercent($n_matchId, $n_playerId)
    {
        $s_sql = 'SELECT
                    IFNULL(
                        ROUND(
                            (
                                sel_count/(SELECT COUNT(user_team_id) FROM user_team WHERE match_id = '.$n_matchId.')
                            )*100, 
                            2
                        ), 
                        ""
                    ...
                    IFNULL(
                        ROUND(
                            (
                                sel_count_captain/(SELECT COUNT(user_team_id) FROM user_team WHERE match_id = '.$n_matchId.')
                            )*100, 
                            2
                        ), 
                        ""
                    ) AS sel_by_per_captain,
                    IFNULL(
                        ROUND(
                            (
                                sel_count_vcaptain/(SELECT COUNT(user_team_id) FROM user_team WHERE match_id = '.$n_matchId.')
                            )*100, 
                            2
                        ), 
                        ""
                    ) AS sel_by_per_vcaptain
                FROM API_player_select
                WHERE sel_ply_id = '.$n_playerId.' AND sel_mat_id = '.$n_matchId;
    ...
    public function getPlayerDetail($teamPlayers, $matchId, $teamId='')
    {
        if ($teamPlayers) 
        {
            $sql = 'SELECT 
                        ...
                        ply_title AS title, 
                        ply_shortName AS shortName, 
                        ply_country AS country, 
                        ply_logo_url AS photo, 
                        rol_name AS role, 
                        ply_fantasy_points, 
                        ply_logo_url AS photo, 
                        (CASE WHEN pnt_total IS NULL THEN 0 ELSE pnt_total END) AS points,
                        (CASE WHEN mat_sqd_id != "" THEN 1 ELSE 0 END) AS playing11,
                        IFNULL(ROUND((sel_count/(SELECT COUNT(user_team_id) FROM user_team WHERE match_id = '.$matchId.'))*100, 2), "") AS sel_by_per,
                        IFNULL(ROUND((sel_count_captain/(SELECT COUNT(user_team_id) FROM user_team WHERE match_id = '.$matchId.'))*100, 2), "") AS sel_by_per_captain,
                        IFNULL(ROUND((sel_count_vcaptain/(SELECT COUNT(user_team_id) FROM user_team WHERE match_id = '.$matchId.'))*100, 2), "") AS sel_by_per_vcaptain
                    FROM API_players
                    ...
                    LEFT JOIN API_player_select
                        ON sel_ply_id = ply_id AND sel_mat_id = '.$matchId.'
                    LEFT JOIN API_player_role
                        ON rol_ply_id = ply_id AND rol_mat_id = '.$matchId.'
    ...
    public function getPlayerMatch($plyId, $matchId)
    {
        $s_sql = "SELECT 
                    API_player_point.*,
                    mat_short_title,
                ...
                FROM API_player_point
                INNER JOIN API_matches
                    ON mat_id = pnt_mat_id AND mat_comp_id IN (SELECT mat_comp_id FROM API_matches WHERE mat_id = ".$matchId.")
                WHERE 
                    pnt_ply_id = ".$plyId;
        $query = $this->db->query($s_sql); 
        $result = $query->result_array();
    ...
    public function updatePlayerSelectCount($ply_ids, $matchId, $value, $captainId, $vcaptainId)
    {
        $set_col = ($value == 'INC') ? 'b.sel_count+1' : 'b.sel_count-1';
        $set_col_captain = ($value == 'INC') ? 'b.sel_count_captain+1' : 'b.sel_count_captain-1';
        $set_col_vcaptain = ($value == 'INC') ? 'b.sel_count_vcaptain+1' : 'b.sel_count_vcaptain-1';

        //update count of selected player
        $s_sql = "UPDATE API_player_select a 
                INNER JOIN API_player_select b 
                    ON a.sel_id = b.sel_id 
                SET a.sel_count = ".$set_col." 
                WHERE 
                    a.sel_mat_id = ".$matchId." AND 
                    a.sel_ply_id IN (".implode(",", $ply_ids).")";
        ...
        $s_sql = "UPDATE API_player_select a 
                INNER JOIN API_player_select b 
                    ON a.sel_id = b.sel_id 
                SET a.sel_count_captain = ".$set_col_captain." 
                WHERE 
                    a.sel_mat_id = ".$matchId." AND 
                    a.sel_ply_id = ".$captainId;

        $this->db->query($s_sql);

        //update vice captain count
        $s_sql = "UPDATE API_player_select a 
                INNER JOIN API_player_select b 
                    ON a.sel_id = b.sel_id 
                SET a.sel_count_vcaptain = ".$set_col_vcaptain." 
                WHERE 
                    a.sel_mat_id = ".$matchId." AND 
                    a.sel_ply_id = ".$vcaptainId;

        $this->db->query($s_sql);
    }

    public function getRemoteUser($n_match_id)
    {
        $s_sql = "SELECT 
                    email
                FROM user
                WHERE 
                    status = 3 AND 
                    email NOT IN (SELECT user_id FROM user_team WHERE match_id = ".$n_match_id.") 
                LIMIT 0, 1";
    ...
    public function getRemoteUserTeams($n_match_id, $n_num)
    {
        $s_sql = "SELECT 
                    user_team_id,
                    user_id
                FROM user_team
                INNER JOIN user
                    ON email = user_id AND status = 3
                WHERE 
                    match_id = ".$n_match_id." AND 
                    user_id NOT IN (SELECT user_id FROM matches_joined WHERE match_id = ".$n_match_id.") 
                LIMIT 0, ".$n_num;
    ...
    public function joinContestViaRemoteUser($request)
    {
        $userId = $request['userId'];
        $matchId = $request['matchId'];
        $contestId = $request['contestId'];
